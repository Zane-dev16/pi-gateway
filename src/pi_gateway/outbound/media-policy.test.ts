...
describe("allowed roots collection shape", () => {
	...
	it("enumerates profiles/<name>/cache/<subdir> for every EXISTING profile", () => {
		...
		mustMkdir(join(piHome, "profiles", "play", "cache"));
		const roots = collectAllowedRoots({ env: {}, home, piHome });
		for (const expected of [
			join(piHome, "profiles", "work", "cache", "images"),
			join(piHome, "profiles", "work", "cache", "audio"),
			join(piHome, "profiles", "play", "cache", "videos"),
		]) {
			expect(roots).toContain(expected);
		}
	});

	it("STRICT mode delivers profile-cache media that previously dropped", () => {
		const imgDir = mustMkdir(
			join(piHome, "profiles", "worker", "cache", "images"),
		);
		const img = touch(join(imgDir, "render.png"), STALE); // stale ⇒ recency can't rescue
		expect(
			validateMediaDeliveryPath(
				img,
				deps({ [MEDIA_DELIVERY_STRICT_ENV]: "1" }),
		...
		// Non-cache files under the same profile stay outside the allowlist.
		const docs = touch(join(piHome, "profiles", "worker", "notes.pdf"), STALE);
		expect(
			validateMediaDeliveryPath(
				docs,
				deps({ [MEDIA_DELIVERY_STRICT_ENV]: "1" }),
