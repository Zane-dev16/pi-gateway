// pi_platforms/wecom/wecom-subject — the WeCom callback adapter wired as a
// ConformanceSubject (04 §8 merge-gate wiring). Shared rows run against the
// REAL kit-built adapter with FakePlatformWire egress capture (the proactive
// send door lands there); the encrypted-callback plane is exercised by the
// engine fixture against the adapter's HTTP-handler seams.
...
import { validateWecomTrustBoundary } from "./manifest.js";
...
export interface WecomSubjectOptions {
 ...
 * user-visible transmission into FakePlatformWire.ops as PROACTIVE sends.
 */
function makeCaptureTransport(raw: FakePlatformWire) {
	return {
		async getAccessToken(): Promise<{ token: string; expiresIn: number }> {
			return { token: "subject-token", expiresIn: 7200 };
		},
		async sendMessage(
			_app: unknown,
			_token: string,
...
/** The WeCom-shaped ConformanceSubject over the REAL adapter. */
export class WecomCallbackSubject implements ConformanceSubject {
	...
	constructor(opts: WecomSubjectOptions) {
		this.name = opts.name ?? "wecom-callback";
		this.wire = opts.wire;
		this.adapter = new WecomCallbackAdapter({
			...
			secretReader: () =>
				opts.withSecret === false ? undefined : FIXTURE_WECOM_TOKEN_A,
			config: {
	...
	// ── streaming seam (proactive-send only; no native lanes) ──
	...
	missingSecretSubjectLifecycle(): AdapterStatusSnapshot {
		const sibling = new WecomCallbackAdapter({
	...
	flagsAndTrustProbe(): {
		...
		const errors = validateWecomTrustBoundary(this.adapter.trustBoundary);
		return {
...
export function makeWecomSubject(
	opts: WecomSubjectOptions & { wire: FakePlatformWire },
): WecomCallbackSubject {
	return new WecomCallbackSubject(opts);
}
