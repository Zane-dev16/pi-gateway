...
update pipeline. Behavior here is specified in spec 08 (Operations) with
parity anchors; citations below refer to that document and the DEC log.
...
manifest-driven adapters → signal handlers. Degraded start is allowed per
service but is always loud; a missing platform secret disables that adapter
loudly instead of skipping it.

Stopping drains in order: stop ingress → active turns finish within the
grace window → release leases → flush delivery obligations, token rollups,
pending messages → close DB → exit. Non-zero exit only on failed flushes
(spec 08 §1.2).
...
processes, SIGTERMs it (≤10s bounded wait, then force), and the old process
recognizes a covered takeover and exits 0. If the marker can't be used,
startup fails without griefing an unrelated process.

## Health and status

- **`gateway_state.json`** under `PI_HOME` is rewritten on every status
  transition: pid, argv, start time, state (`starting | running | draining |
  stopped …`), active agent count, per-platform connection state, and the
  `code_sha`/`code_version` stamps the update pipeline verifies against
  (spec 08 §4).
- **`state/gateway.heartbeat`** is rewritten every 30s; a stale mtime means
  "process alive, loop frozen" — the external monitor's signal.
- **`/status`** (chat command) reports adapters up/disabled-with-reason,
...
| `errors.log`         | WARNING+ with reason codes (authz denials, lease loss, retries) — first triage stop |
...
Three independent backstops (spec 08 §1.3): **flush** serializes pending
messages and unflushed agent history to recovery files under
`pending_messages/` before memory is discarded (replayed on next boot);
**forensics** writes a <10 ms context probe plus a detached diagnostic walk
so an invisible killer (container/OOM) leaves evidence; the **watchdog**
hard-exits a wedged drain after dumping all thread stacks. Locks and the PID
file are always released before a hard exit.
...
- **Plan** inventories runtimes and refuses non-in-place kinds (docker/nix/
  apt) with the right external command, exit 1, before mutating anything.
- **Snapshot** writes per-profile state snapshots (identical critical set,
  1 GiB per-file cap, keep policy floors at 1) — file-loss recovery, not
  code rollback.
...
- **No live config reload** (DEC-013): SIGHUP is not a reload signal; restart
  via `--replace` to apply changes. Update runs install hangup protection so
  package children survive terminal loss (DEC-042).
- **Process identity is never guessed from argv substrings** — canonical,
  parser-derived matchers only (spec 08 §9).
- **Token locks**: a unique credential (e.g. one bot token) can be held by
  ...
  profile + PID (spec 06 §5).
- **Cron** is bound by inactivity (default 600s), not wall-clock — active
  jobs may run for hours (spec 07 §5.2).
