...
Environment/layout basics are in [docs/installation.md](installation.md).

## Two kinds of input

1. **Behavior** comes from the config file in the profile home.
2. **Secrets** come from the `.env` store; adapters also read well-known env
   vars for authorization knobs (below).

Config is read once at boot per watcher/service. **There is no live reload**
in v0.1 — SIGHUP is not a reload signal (DEC-013); restart the gateway to
apply changes (see [docs/operations.md](operations.md) for the takeover
handshake that makes this cheap).
...
- Under **multiplex** (several profiles in one process), each inbound turn
  runs under a per-turn scope pair — a home redirect plus a secret scope,
  reset in reverse order even on error. Env reads on scoped paths **fail
  closed**: a scoped miss returns the declared default instead of falling
  back to the raw process environment, which could leak another profile's
  credentials or allowlists (spec 06 §3, DEC-003/009). The ban is
  grep-gated in CI (`npm run check:secrets`).
...
Adapters may default to `pairing` (e.g. DM pairing handshake with a code,
lockout after 5 failed approvals, approval written into the allowlist —
spec 06 §2.4). A message that arrives under an adapter's `open` policy is
**not** trusted as authorization: the gateway trusts adapter decisions only
when the effective policy is `allowlist` (the #34515 fail-open fix). Webhook
platforms additionally verify HMAC signatures (±300s replay window for
Svix-style schemes) and may require IP-range allowlists on non-loopback binds
(spec 06 §8, DEC-017).

## Gateway behavior knobs

- **Busy behavior** is registry-driven, not configurable per platform:
...
- **Worker pool** is bounded (10 workers); correctness comes from leases, not
  thread counts (spec 01 §2.1).
