...
installed before any project import, and all state reads/writes go through
...
- A **planned takeover** (`--replace`) makes the old process exit **0** on
  purpose, so `Restart=on-failure` does not flap-fight the replacer.
...
restart-per-kind → verify → receipt — and a mixed-version fleet fails
verification with exit 1 rather than reporting healthy (spec 08 §8). Receipts
land in `logs/update_receipts/`. Details and operator procedures:
