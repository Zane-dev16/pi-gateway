...
Before filing, check [docs/troubleshooting.md](docs/troubleshooting.md); many
startup, delivery, and configuration questions are answered there. Search
existing issues too.
...
1. What you ran: the command line, `pi gateway run` or a client surface, and
...
3. Platform surface: e.g. Telegram, Discord, Slack, and whether the failure
   was inbound (receiving) or outbound (delivery).
4. Logs: the relevant tail of `logs/errors.log` and `logs/gateway.log` under
   your `PI_HOME`. Reason codes in denial/retry lines are especially useful.
...
Please do not paste bot tokens, pairing codes, or message content from
