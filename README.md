...
serialized, durable agent turns against a single shared SQLite substrate** —
...
- **31 platform surfaces** across three transport shapes — polling, persistent
  WebSocket, webhook (DEC-002) — all gated by one executable conformance suite
  (spec 04 §8).
...
- **Streaming with a contract:** prefix-stable drafts, `finish(final_text)` is
  authoritative, seal-interception at every egress door (DEC-006, spec 04 §5).
- **Durable delivery:** obligations ledger with capped attempts/retention;
  timeout-classified send failures are never blind-retried (DEC-046, DEC-054).
- **One command registry:** busy policies, help, menus, and completions all
  derive from a single frozen registry (spec 07 §1, DEC-005).
- **Fail-closed security:** scoped secrets, deny-by-default authorization,
  pairing, per-credential token locks, signed webhooks (spec 06, DEC-003/009).
- **Transactional operations:** plan → snapshot → apply → restart-per-kind →
  verify → receipt updates; a mixed-version fleet is never "healthy"
  (spec 08 §8).
...
- Before opening a PR, read [CONTRIBUTING.md](CONTRIBUTING.md) — it covers the
  required test gates and the divergence rule (DEC-026).
