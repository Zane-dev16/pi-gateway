// Layer 1 of the two-layer turn lease (02-session-and-state.md §5, DEC-004):
// the in-process, single-event-loop lease registry with generation-scoped,
// identity-checked release.
//
// Semantic port of hermes gateway/turn_lease.py:SessionTurnLeaseRegistry
// (READ-ONLY reference; semantics only, no code vendored). Anchors:
...
//   resolving the waiter's promise, so there is no observable handoff window;
//   pendingAcquires is still tracked for queued waiters to keep _evict_idle
//   parity (a session with a pending acquire is never evictable).
...
/** gateway/turn_lease.py:DEFAULT_MAX_LEASES */
